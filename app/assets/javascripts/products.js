$(function()  {
  ...
  //ドラッグアンドドロップ
  //領域に入ったとき
  $(document).on('dragenter', ".input", function(){
    $(".input").css('border', '1px solid greenyellow');
  });
  //領域から出たとき
  $(document).on('dragleave', ".input", function(){
    $(".input").css('border', '1px dashed rgb(204, 204, 204)');
  });
  //領域上にあるとき
  $(document).on('dragover', ".input", function(e){
    e.preventDefault();
  });
  // ドロップした時
  $(document).on('drop', ".input", function(e){
    ...
    $(".input-area").children('.image_upload')[0].files = e.originalEvent.dataTransfer.files;
    ...
    $ul = $("#previews");
    $li = $("#previews li").last();
    $label = $(this).children(".upload-label");
    $inputs = $ul.find(".image_upload");
    ...
    var append_input = $(
     ...
     reader.onload = function (e) {
        ...
        //previewをappendで追加する前に、プレビューできるように画像を<img>に'src'で付与させる
        $(preview).find(".preview").attr("src", e.target.result);
      };
